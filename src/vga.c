...
void delete() {
    if (vga_index < VGA_BUFFER_SIZE) {
        for (int i = vga_index; i < VGA_BUFFER_SIZE - 1; i++) {
...
void vga_bounds_check(unsigned char color) {
    ...
    if (vga_index >= VGA_BUFFER_SIZE) {
            ...
            */
            vga_index = VGA_BUFFER_SIZE - VGA_WIDTH;
    }
}
    ...
    while (str[i]) {
        ...
        if (str[i] == '\n') {
            ...
            if (str[i] == '\t')
                ...
                vga_buffer[vga_index++] = (unsigned short)character | (unsigned short)color << 8;
        }
...
void print_char(char c, unsigned char color) {
    ...
    if (c == '\n')
        ...
        vga_buffer[vga_index++] = (unsigned char)c | (unsigned short)color << 8;
    vga_bounds_check(color);
    move_cursor();
}
